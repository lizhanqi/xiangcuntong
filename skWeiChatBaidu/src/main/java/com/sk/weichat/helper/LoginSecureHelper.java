            ...
            LoginSecureHelper.generateAutoLoginParam(
                    ...
                    (data, loginToken, loginKeyData, salt) -> {
                        ...
                        HttpUtils.get().url(coreManager.getConfig().USER_LOGIN_AUTO)
                                .params(p)
                                .build(true, true)
                                .refreshAccessTokenSync(new BaseCallback<EncryptedData>(EncryptedData.class, false) {
                                    @Override
    ...
    public static void secureLogin(
        ...
        if (!TextUtils.isEmpty(thirdToken)) {
            ...
            if (TextUtils.equals(LoginActivity.THIRD_TYPE_WECHAT, thirdTokenType)) {
                ...
                if (coreManager.getConfig().isNoRegisterThirdLogin) {
                    params.put("nickName", QQHelper.parseNickName(thirdToken));
                    params.put("sex", QQHelper.parseSex(thirdToken));
                    params.put("headImageUrl", QQHelper.parseHeadimgUrl(thirdToken));
                }
            } else {
