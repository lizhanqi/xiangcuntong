...
public class FriendHelper {
        ...
        if (attentionUser == null) {// 服务器上不存在关系
            ...
            if (friend != null) {// 服务器上不存在关系，本地却有
                if (friend.getStatus() != Friend.STATUS_23
                        && !(TextUtils.equals(friend.getUserId(), Friend.ID_SYSTEM_MESSAGE)
                        || TextUtils.equals(friend.getUserId(), Friend.ID_SK_PAY))) {
                    // 从好友表中改变状态
            ...
            if (friend == null) {// 本地不存在关系，那么就要插入一条好友记录
                ...
                friend.setTimeSend(attentionUser.getCreateTime());
                friend.setRoomFlag(0);// 0朋友 1群组
                ...
                if (attentionUser.getStatus() == 1) {
                    // 打招呼对方没回复时会是1，当成陌生人，避免在消息列表出现，
                    status = Friend.STATUS_23;
                }
    ...
    public static void beBlacklistExtraOperation(String loginUserId, String friendId) {
        ...
        FriendDao.getInstance().updateFriendContent(loginUserId, friendId, null, new ChatMessage());
    ...
    public static void removeAttentionOrFriend(String ownerId, String friendId) {
        ...
        FriendDao.getInstance().updateFriendContent(ownerId, friendId, null, new ChatMessage());
