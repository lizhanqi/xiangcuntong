    ...
    public void getLastChatHistory() {
        ...
        if (Constants.OFFLINE_TIME_IS_FROM_SERVICE) {// 离线时间为服务端获取 取出消息漫游时长
            ...
            if (realSyncTime == -2) {// 不同步
                ...
                // 这里不能直接赋值0，否则清空本地之后，退出重新登录，清空的消息又拉出来了
                // 这里也不能直接赋予offlineTime，否则卸载重装，登录之后就什么都没有了，和ios一样本地也记录一下，就第一次传0
                boolean isUsed = PreferenceUtils.getBoolean(MyApplication.getContext(), Constants.OFFLINE_TIME_SINGLE_USE + mLoginUserId, false);
                // 使用完之后就标记为true
                PreferenceUtils.getBoolean(MyApplication.getContext(), Constants.OFFLINE_TIME_SINGLE_USE + mLoginUserId, true);
                syncTimeLen = isUsed ? offlineTime : 0;
            } else {
                // 这里变量名有点混乱，syncTimeLen应该是startTime, 是个时间点，不是时间段，
                syncTimeLen = TimeUtils.sk_time_current_time() - (long) (realSyncTime * 24 * 60 * 60);// 得到消息同步时长
                syncTimeLen = Math.max(syncTimeLen, offlineTime);
            }
    ...
    public void notificationMessage(ChatMessage chatMessage, boolean isGroupChat) {
        ...
        switch (messageType) {
            ...
            case XmppMessage.TYPE_DICE:
                content = getString(R.string.type_dice);
                break;
            case XmppMessage.TYPE_RPS:
                content = getString(R.string.type_rps);
                break;
