...
public class PhotoPreviewActivity extends SwipeBackActivity implements PhotoPagerAdapter.PhotoViewClickListener {
    ...
    public static final String EXTRA_REAL_PHOTOS = "extra_real_photos";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ArrayList<String> realPhotos = getIntent().getStringArrayListExtra(EXTRA_REAL_PHOTOS);
        if (pathArr != null) {
            for (int i = 0; i < pathArr.size(); i++) {
                PhotoPagerAdapter.Item item = new PhotoPagerAdapter.Item();
                item.path = pathArr.get(i);
                if (realPhotos != null && realPhotos.size() > i && !TextUtils.equals(item.path, realPhotos.get(i))) {
                    item.changed = true;
                    item.resultPath = realPhotos.get(i);
                }
                paths.add(item);
            }
        }
