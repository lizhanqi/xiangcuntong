...
//        mRbTab5.setVisibility(!TextUtils.isEmpty(coreManager.getConfig().homeAddress) ? View.VISIBLE : View.GONE);
        findViewById(R.id.flHomePage).setVisibility(!TextUtils.isEmpty(coreManager.getConfig().homeAddress) ? View.VISIBLE : View.GONE);
        ...
        isCreate = false;
        //  修改白屏bug
        if (!TextUtils.isEmpty(coreManager.getConfig().homeAddress)) {
            mRbTab1.toggle();
        } else {
           mRbTab1.toggle();//由于开启后选中标签在第四页所以屏蔽
        }
        // initFragment();

        // 改皮肤，
        ColorStateList tabColor = SkinUtils.getSkin(this).getMainTabColorState();
        for (RadioButton radioButton : Arrays.asList(mRbTab1, mRbTab2, mRbTab3, mRbTab4, mRbTab6)) {//mRbTab5
            // 图标着色，兼容性解决方案，
