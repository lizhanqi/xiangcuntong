    ...
    class My_BroadcastReceivers extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(com.sk.weichat.broadcast.OtherBroadcast.singledown)) {
                ...
                if (mRealImageUrl.contains("http")) {
                    ImageLoadHelper.loadBitmapDontAnimate(mContext, mRealImageUrl, b -> {
                        Bitmap bitmap = BitmapUtil.getBitmapQRCode(MultiImagePreviewActivity.this, b);
                        mSaveWindow = new SaveWindow(MultiImagePreviewActivity.this,
                                bitmap != null, new ClickListener(b));
                        mSaveWindow.show();
                    }, e -> {
                        // todo load bitmap failed
                    });
                } else {
                    Bitmap bitmap = BitmapUtil.getBitmapQRCode(MultiImagePreviewActivity.this, mRealImageUrl);
                    mSaveWindow = new SaveWindow(MultiImagePreviewActivity.this,
