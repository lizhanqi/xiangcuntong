...
public class PrivacySettingActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void submitPrivacySetting(final int index, final boolean isChecked) {
        setSwitchButtonEnableTouch(index, false);
        Map<String, String> params = new HashMap<>();
        params.put("access_token", coreManager.getSelfStatus().accessToken);
        params.put("userId", mLoginUserId);
        ...
        HttpUtils.get().url(coreManager.getConfig().USER_SET_PRIVACY_SETTING)
                ...
                .execute(new BaseCallback<Void>(Void.class) {
                    ...
                    @Override
                    public void onResponse(ObjectResult<Void> result) {
                        setSwitchButtonEnableTouch(index, true);
                        if (Result.checkSuccess(PrivacySettingActivity.this, result)) {
                            Toast.makeText(PrivacySettingActivity.this, getString(R.string.update_success), Toast.LENGTH_SHORT).show();
                            PrivacySetting privacySetting = PrivacySettingHelper.getPrivacySettings(PrivacySettingActivity.this);
    ...
    private void setSwitchButtonEnableTouch(int type, boolean enableTouch) {
        if (type == 1) {
            ...
            mSbEncrypt.setEnableTouch(enableTouch);
        } else if (type == 3) {
            mSbVibration.setEnableTouch(enableTouch);
        } else if (type == 4) {
            mSbInputState.setEnableTouch(enableTouch);
        } else if (type == 5) {
            mSbUseGoogleMap.setEnableTouch(enableTouch);
        } else if (type == 6) {
            mSbSupport.setEnableTouch(enableTouch);
        } else if (type == 7) {
            mSbKeepLive.setEnableTouch(enableTouch);
        } else if (type == 8) {
            sbPhoneSearch.setEnableTouch(enableTouch);
        } else if (type == 9) {
            sbNameSearch.setEnableTouch(enableTouch);
        } else if (type == 10) {
            mSbAuthLogin.setEnableTouch(enableTouch);
        } else if (type == 11) {
            mSbPrivacyPosition.setEnableTouch(enableTouch);
        } else if (type == 12) {
            mSbSlideClearServer.setEnableTouch(enableTouch);
        } else if (type == 13) {
            mSbShowMsgState.setEnableTouch(enableTouch);
        }
    }
