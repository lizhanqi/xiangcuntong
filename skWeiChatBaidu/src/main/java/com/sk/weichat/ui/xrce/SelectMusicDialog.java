...
public class SelectMusicDialog extends Dialog implements View.OnClickListener {
    ...
    TextWatcher changeListener = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            if (s.length() > 0) {
                ...
                cancelSearchTimer();
                if (mCurrMusic != null) {
                    for (MusicInfo info : mdatas) {
                        if (TextUtils.equals(info.id, mCurrMusic.id)) {
                            info.setState(mCurrMusic.state);
                        } else {
    ...
    private void playMusic(int position) {
        ...
        if (mCurrPlay == position || (mCurrMusic != null && TextUtils.equals(mCurrMusic.id, data.get(position).id))) {
            MusicInfo clickMusic = data.get(position);
            mCurrMusic = clickMusic;
                        ...
                        if (Result.checkSuccess(mContext, result)) {
                            ...
                            if (data.size() > 0) {
                                for (int i = 0; i < data.size(); i++) {
                                    ...
                                    if (mCurrMusic != null && TextUtils.equals(mCurrMusic.id, info.id)) {
                                        info.setState(mCurrMusic.state);
                                    }
                                }
